...
class DBUtil {
  ...
  open() async {
    try {
      db = await DbManager.instance!.getCurrentDatabase();
      Log.d('DBUtil open');
    } catch (e) {
      Log.d('DBUtil open() Error $e');
    }
  }
  ...
  Future<int> updateByHelper(String tableName, Map<String, Object?> setArgs,
      String whereStr, List whereArgs) async {
    return db!.update(tableName, setArgs, where: whereStr, whereArgs: whereArgs);
  }
  ...
  Future<int> deleteByHelper(
    ...
    return db!.delete(tableName, where: whereStr, whereArgs: whereArgs);
  }
